...
const PRESETS: Preset[] = [
  ...
  { group: 'Sistema', label: 'Atualizar settings', method: 'PUT', path: '/settings', body: '{ "webhookGlobal": { "enabled": false, "url": "" }, "rateLimit": { "perSec": 1, "burst": 5 }, "device": { "client": "WAMux", "browser": "Chrome" } }' },
  // Instâncias
  { group: 'Instâncias', label: 'Listar', method: 'GET', path: '/instances' },
  { group: 'Instâncias', label: 'Criar', method: 'POST', path: '/instances', body: '{ "name": "teste-01", "provider": "baileys" }' },
  ...
  { group: 'Instâncias', label: 'Trocar engine', method: 'POST', path: '/instances/{id}/provider', body: '{ "provider": "whatsmeow", "migrate": true }' },
  { group: 'Instâncias', label: 'Settings da instância', method: 'PUT', path: '/instances/{id}/settings', body: '{ "identityMode": "auto" }' },
  { group: 'Instâncias', label: 'Remover', method: 'DELETE', path: '/instances/{id}' },
  // Eventos & Webhook
  { group: 'Eventos', label: 'Webhook', method: 'PUT', path: '/instances/{id}/webhook', body: '{ "url": "https://exemplo.com/hook", "events": [] }' },
  { group: 'Eventos', label: 'Eventos (3 transportes)', method: 'PUT', path: '/instances/{id}/events', body: '{ "webhook": { "enabled": true, "url": "https://exemplo.com/hook", "events": [] }, "websocket": { "enabled": false, "events": [] }, "rabbitmq": { "enabled": false, "events": [] } }' },
  { group: 'Eventos', label: 'Webhook DLQ', method: 'GET', path: '/instances/{id}/webhook/dlq' },
  { group: 'Eventos', label: 'Retry DLQ', method: 'POST', path: '/instances/{id}/webhook/dlq/retry' },
  // Filtros & Anti-ban
  { group: 'Anti-ban', label: 'Filtros JID', method: 'PUT', path: '/instances/{id}/filters', body: '{ "allowJids": [], "blockJids": ["5511888888888"], "direction": "both" }' },
  { group: 'Anti-ban', label: 'Perfil anti-ban', method: 'PUT', path: '/instances/{id}/anti-ban', body: '{ "profile": "normal" }' },
  { group: 'Anti-ban', label: 'Status anti-ban', method: 'GET', path: '/instances/{id}/anti-ban/status' },
  // Mensagens
  ...
  { group: 'Mensagens', label: 'Lista', method: 'POST', path: '/messages/{id}/list', body: '{ "to": "5511999999999", "text": "Escolha uma opção", "buttonText": "Ver opções", "sections": [{ "title": "Seção 1", "rows": [{ "id": "1", "title": "Opção 1" }] }], "fallbackToText": true }' },
  { group: 'Mensagens', label: 'PIX', method: 'POST', path: '/messages/{id}/pix', body: '{ "to": "5511999999999", "pix": { "key": "chave@email.com", "keyType": "email", "merchant": "Loja X" }, "fallbackToText": true }' },
  { group: 'Mensagens', label: 'Status de entrega', method: 'GET', path: '/messages/{id}/status/{messageId}' },
  { group: 'Mensagens', label: 'Resultado da enquete', method: 'GET', path: '/messages/{id}/poll/{messageId}' },
  { group: 'Mensagens', label: 'Status na fila', method: 'GET', path: '/messages/{id}/queue/{jobId}' },
  { group: 'Mensagens', label: 'Baixar mídia', method: 'GET', path: '/messages/{id}/media/{messageId}' },
  // Etiquetas
  { group: 'Etiquetas', label: 'Listar', method: 'GET', path: '/instances/{id}/labels' },
  { group: 'Etiquetas', label: 'Criar/editar', method: 'POST', path: '/instances/{id}/labels', body: '{ "name": "Cliente VIP", "color": { "index": 0 } }' },
  { group: 'Etiquetas', label: 'Remover', method: 'DELETE', path: '/instances/{id}/labels/{labelId}' },
  { group: 'Etiquetas', label: 'Associar a chat/contato', method: 'PUT', path: '/instances/{id}/labels/{labelId}/associations', body: '{ "targetType": "chat", "targetId": "5511999999999@s.whatsapp.net", "on": true }' },
  { group: 'Etiquetas', label: 'Chats da etiqueta', method: 'GET', path: '/instances/{id}/labels/{labelId}/chats' },
  { group: 'Etiquetas', label: 'Etiquetas do contato', method: 'GET', path: '/instances/{id}/contacts/{jid}/labels' },
  { group: 'Etiquetas', label: 'Etiquetas do chat', method: 'GET', path: '/instances/{id}/chats/{jid}/labels' },
  // Contatos
  { group: 'Contatos', label: 'Checar números', method: 'POST', path: '/instances/{id}/numbers/check', body: '{ "numbers": ["5511999999999"] }' },
  { group: 'Contatos', label: 'Presença (enviar)', method: 'POST', path: '/instances/{id}/presence', body: '{ "to": "5511999999999", "state": "composing", "durationMs": 2000 }' },
  { group: 'Contatos', label: 'Presença do contato', method: 'GET', path: '/instances/{id}/contacts/{jid}/presence' },
  { group: 'Contatos', label: 'Bloquear', method: 'POST', path: '/instances/{id}/contacts/{jid}/block' },
  { group: 'Contatos', label: 'Desbloquear', method: 'POST', path: '/instances/{id}/contacts/{jid}/unblock' },
  { group: 'Contatos', label: 'Mensagens do chat', method: 'GET', path: '/instances/{id}/chats/{jid}/messages' },
  { group: 'Contatos', label: 'Marcar como lido', method: 'POST', path: '/instances/{id}/chats/{jid}/read' },
  // Canais
  { group: 'Canais', label: 'Listar', method: 'GET', path: '/instances/{id}/newsletters' },
  { group: 'Canais', label: 'Criar', method: 'POST', path: '/instances/{id}/newsletters', body: '{ "name": "Meu Canal", "description": "Novidades do WAMux" }' },
  ...
  { group: 'Histórico', label: 'Status do import', method: 'GET', path: '/instances/{id}/history/import/{jobId}' },
  { group: 'Histórico', label: 'Cancelar import', method: 'POST', path: '/instances/{id}/history/import/{jobId}/cancel' },
];
